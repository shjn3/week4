...
export class GameScene extends Phaser.Scene {
  ...
  private emitterBall: Phaser.GameObjects.Particles.ParticleEmitter;
  ...
  private throwBall(): void {
    if (this.canFire) {
      ...
      this.emitterBall = this.add.particles("ball").createEmitter({
        blendMode: "ADD",
        speed: 100,
        alpha: { start: 0, end: 1 },
        scale: { start: 0.6, end: 0 },
      });
      this.emitterBall.startFollow(this.ball);
  ...
  private handleBallClockOverlap(ball: Ball, clock: Clock) {
    if (!this.canFire) {
      if (clock) {
        this.add.tween({
          targets: [clock, clock.getFace(), clock.getHand()],
          duration: 200,
          ease: "Power1",
          yoyo: true,
          props: {
